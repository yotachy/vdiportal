...
`spend` 성공 후 분석이 던지면 **`refund(idem)`** 하고 사용자에게 알린다. 낙관적 차감을 하지 않는다는 `SPEC §1` 의 요구를 클라이언트 쪽에서 지키는 방법이다.
...
| `www/screens/report.js` | 티어 상태 · 시트 진입 · Full 재실행 · 필 | 배선 |
...
- **주/월 매트릭스** — Full 의 시안 정의(`All 32 · daily, weekly, monthly`)에 포함되나 이번은 **일봉 32지표까지**. 주/월은 `loadTicker` 2회 + 엔진 2회가 더 붙고 주기 행 UI 재작업이 필요하다
...
- **Full 실행 시간이 미실측이다.** Phase 1 실측은 *"Basic 5지표는 데스크톱 5031봉 약 20.2ms, Full 32지표 대비 약 128배 저렴"* — 데스크톱에서 Full 이 약 2.6초라는 뜻이다. 모바일은 봉 수가 훨씬 적지만 실측이 없다. 구현 중 폰에서 재고, 체감되면 시트에 진행 표시를 붙인다
