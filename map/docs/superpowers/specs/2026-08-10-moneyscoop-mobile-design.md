...
| 실기기가 갤럭시 Z폴드7 하나 | ① 플래그십이라 성능 측정치가 낙관적 → ~~데스크톱 Chrome CPU 6배 스로틀을 중급기 대리치로~~ **(2026-08-10 폐기: 스로틀 배수는 기기 성능의 대용이 못 된다 — 폴드7이 이 데스크톱보다 3.3배 빨랐다. 중급기 보정은 배수 가정이 아니라 봉 수 절감으로 다룬다)** ② 폴더블이라 600–904dp 2단 레이아웃이 "나중 일"이 아니라 주력 화면 |
...
**판정 기준**: Full(32지표 × 3타임프레임)이 **2초를 넘으면 Web Worker + 지표별 진행률 확정**. 이 결정을 여기서 내리지 않으면 두 달 뒤 화면 전체를 다시 짠다.

**결과(2026-08-10 · 폴드7 커버화면 Chrome)**: 콜드 776.5ms / 반복 1074.4ms → **Worker 조건부 불필요.** 조건은 **일봉 히스토리 절감** — 실행시간이 봉 수에 초선형(약 O(n^1.5))이라 1day 5031봉이 합계의 75%를 먹고, 중급기(단일스레드 2~3배 느림)에서는 같은 봉 수면 임계를 넘는다. 1200봉 수준으로 줄이면 3주기 약 320ms. **Phase 1 이 답할 것은 "Worker 를 넣을까"가 아니라 "주기별로 몇 봉에서 정확도가 꺾이는가"다.** 전체 기록: `map/mobile/docs/phase0-measurements.md`
