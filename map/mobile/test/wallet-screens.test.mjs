...
test("report.js 소스 모양 — purchaseFull 이 이전 시도의 idem 을 재사용한다(새로 뽑지 않는다)", () => {
  // 이전 rec.idem 이 있으면 그걸 쓰고, 없을 때만 새로 뽑는다. "항상 새로 뽑는다"(구버전)로
  // 되돌아가면 maybe-charged 재시도가 원장에서 별개 키가 되어 이중 차감된다.
  assert.match(REPORT, /var idem = \(rec && rec\.idem\) \? rec\.idem : MSWallet\.newIdem\(\);/,
...
test("watchlist.js 소스 모양 — beginScan 이 pendingScanIdem 을 재사용하고, maybe-charged 실패에만 다시 채운다", () => {
  assert.match(WATCHLIST, /var pendingScanIdem = null;/, "모듈 스코프 pendingScanIdem 이 없다");
  assert.match(WATCHLIST, /var idem = pendingScanIdem \|\| MSWallet\.newIdem\(\);/,
    "beginScan 이 이전 실패의 idem 을 재사용하지 않는다");
  assert.match(WATCHLIST, /if \(MSWallet\.maybeCharged\(sp\.reason\)\) pendingScanIdem = idem;/,
