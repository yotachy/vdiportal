...
const ALLOWED_LATIN = [
  ...
  // 리뷰 Minor(Task 5, 1/5) — 이 항목이 지키던 obRisk(위험고지)를 Task 5 가 지웠다(3단계를
  // 성향으로 재설계). 지금은 strings.js 어떤 값도 "MoneyScoop"을 안 쓴다 — 그래도 항목은
  // 지우지 않는다: Task 6 이 새 4단계(동의)에서 obRisk 와 같은 취지의 문구("MoneyScoop 은
  // 가격·거래량·시간만 읽습니다…")를 다시 쓸 가능성이 크고, 그때 이 항목이 없으면 같은
  // 브랜드명이 다시 미번역으로 잡힌다. 앱 이름 자체는 언제나 고유명사라 번역 대상이 아니다.
  "MoneyScoop",
...
const KEY_ALLOWED_LATIN = {
  ...
  // 3단 대조(설계서 §3.3) "…%p 오릅니다/낮아집니다" · "%p"(오차 단위) — %p(퍼센트포인트)는
  // 상위 설계서 §9 가 이미 쓰는 표기 그대로다("실측은 +0.3%p"). 국내 금융 UI 도 %p 를
  // 그대로 쓴다(관용 약어, ID·Lv 와 같은 종류). [리뷰 I3] 방향 동사를 부호로 가르면서
  // 조각이 tcDeltaD 하나에서 tcDeltaUp/tcDeltaDown 둘로, ECE 단위가 tcEceUnit 으로 늘었다.
  tcDeltaUp: ["p"], tcDeltaDown: ["p"], tcEceUnit: ["p"],
};
