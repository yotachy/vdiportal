...
// ⚠ 심화는 아직 시안 18b 의 8블록이 **아니다.** 지금은 P1 이 남긴 12개 구성(정보 블록 9개)을
// 그대로 선언으로 옮겼을 뿐이고, 18b 순서로 재구성하는 것은 Task 7(심화분석 18b + 8a 대조)이다.
// 여기서 8 을 단정하면 지금 빨갛고, 현재 값을 주석 없이 적으면 다음 사람이 그것을 목표로 읽는다.
// 그래서 **현재 값을 명시하고 목표를 함께 적는다** — Task 7 이 이 숫자를 8 로 바꾸며 이 줄도 고친다.
test("심화 블록 수 — 현재 10(P1 구성 + 판독문 링크), 목표 8(시안 18b). Task 7 이 맞춘다", () => {
  assert.strictEqual(B.countOf("full"), 10,
    "심화 블록 수가 10 이 아니다. 18b 재구성을 했다면 이 단정을 8 로 바꾸고 주석을 지울 것: " +
    B.orderOf("full").join(" → "));
});
