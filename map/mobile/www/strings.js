...
(function (root, factory) {
  ...
  var t = {
    ...
    // 얻는 자기 오버라인 + 숫자보다 먼저 오는 한 줄. 실측(28창): 심화가 더 좁은 사례 0.0%,
    // 폭 비율 중앙값 1.78배 — 사용자는 돈을 낸 직후 **넓어진** 범위를 본다. "좁아진다"는
    // 반대 사실이라 절대 안 쓴다(report-blocks.test.mjs 가 그 문구가 안 섞이는지 잠근다).
    rpCompareHead: "직전 대비",
    rpCompareWider: "기본분석보다 범위가 넓어졌습니다 — 더 많은 지표를 반영해 정직하게 넓힌 값입니다.",
    ...
    obBack: "뒤로", obNext: "다음",
    // 2026-08-19 리뷰: "예시 데이터"만으로는 골드 헤드라인("맞히셨습니다!") 옆에서 존재감이
    // 없다 — 위치를 헤드라인 위(맨 먼저 읽히는 자리)로 올리고, 문구도 "선택된 예시"라는
    // 사실을 명시했다(과장·자학 없이 사실 진술 한 줄, 컨트롤러 판정 문구를 그대로 옮김).
    obSampleNote: "설명을 위해 고른 예시 구간입니다",
    ...
    obCostFull: "심화분석",
    // obH5/obRisk/obAgree(옛 3단계 위험고지·약관 체크박스)는 Task 5 가 3단계를 성향으로
    // 재설계하며 지웠다 — 이 자리 이름을 재사용하지 않는다(Task 4 의 obH2 전례와 같다).
    // Task 6 이 새 4단계(동의)에서 다시 짓는다.
    ...
    // 이 순간에 돈다). 판정 한 줄 + 세 갈래. 리뷰 C2 — 갈래 라벨은 더 이상 이 화면만의
    // 것(obReadForHead·obReadFlatHead, 삭제됨)이 아니다. 2·3·6단계와 같은 통 이름
    // (ob32AgreeHead·rpAgainst·ob32FlatHead)을 그대로 쓴다 — 같은 지표·같은 구간이 화면마다
    // 다른 이름의 통에 담기면 "계산을 연다"는 이 브랜치의 약속이 화면마다 다른 답을 내는
    // 꼴이 된다.
    ...
    ob32Collapse: "접기",
    // 리뷰 C1 — 2→3 다리. 설계서가 지정한 세 다리(1→2 obAppSawA/B · 2→3 이 줄 · 3→4
    // obPastDone→obFutureOver) 중 이것만 빠져 있었다. 3단계(성향)를 벌어들인다 —
    // "32개를 다 봤으니 이제 무엇을 중시하는지 골라 보라"는 전환.
    obStyleBridgeNote: "도구마다 중요도가 다릅니다. 무엇을 가장 중요하게 보시나요?",

    // 3단계(재설계, 설계서 §4.3, Task 5) — 성향을 고르면 같은 구간을 그 방식으로 다시
    // 판정한다. Task 1 실측(확정 표본 PG): trend·reversion·volatility 는 32도구와 같은
    // 결론(bull)이고 momentum 만 갈린다(neutral, 경계 ±12에서 score 11) — 그래서 "같음"이
    // 이 화면의 주 경로다(obSameNote). 선택 지표 수·근거 구성(동의/반대/무판정/자백)은
    // 4종 모두 다르므로 판정이 같아도 "고르면 갱신된다"는 항상 성립한다.
    // 리뷰 C1 — obPastDone("여기까지는 과거였습니다")은 3단계를 **닫는** 줄이다(§4.3).
    // 예전엔 이 자리(3단계 맨 위)에서 열렸다 — 아직 과거 구간을 보여주는 한복판에서 "과거
    // 였습니다"가 울렸다. 여는 자리는 obStyleOpen 이 대신하고, obPastDone 은 이 화면의
    // 본문 끝(3단계가 실제로 끝나는 지점)으로 옮긴다.
    obStyleOpen: "이제 당신의 기준을 더합니다",
    obPastDone: "여기까지는 과거였습니다.",
    obH3: "당신의 방식으로 다시 봅니다",
    // 리뷰 B(1/5) — "언제든 바꿀 수 있습니다"만으로는 눌러볼 유인이 없다. "판정 자체가
    // 달라질 수도 있습니다"를 더한다 — 어느 성향이 실제로 갈리는지는 말하지 않는다(표본이
    // 바뀌면 거짓말이 된다, 리뷰 지시).
    obSub3: "투자 성향을 고르면, 같은 구간을 그 방식으로 다시 판정합니다. 판정 자체가 달라질 수도 있습니다 — 언제든 바꿀 수 있습니다.",
    // 리뷰 B — "사용"(수동태, 이미 끝난 일처럼 읽힌다) 대신 "다시 판정"(고르는 행위가 곧
    // 새 계산을 일으킨다는 신호)으로 바꿨다. "9개 지표 사용" → "9개 지표로 다시 판정".
    obStyleIndicatorSuffix: "개 지표로 다시 판정",
    // "추세 추종" + 이 접미사 → "추세 추종 기준 판정" — obVerdictLabelSuffix("도구 판정")와
    // 짝을 이루는 두 번째 비교 행 라벨이다.
    obStyleVerdictSuffix: " 기준 판정",
    ob3SameNote: "당신 기준으로도 같은 결론입니다.",
    // 32개 도구는 A + word(32도구) + B + 성향 이름 + C + word(성향) + D. ob32DiffA~D 와
    // 같은 자리에 성향 이름이 n5 대신 들어간다는 점만 다르다.
    ob3DiffA: "32개 도구는 ", ob3DiffB: "라고 봤지만, ", ob3DiffC: " 기준으로 보면 ", ob3DiffD: "입니다.",
    // 리뷰 A(1/5) — 판정이 중립(want===0)이면 voiced 행 전부가 "무판정"으로 간다. 개별
    // 지표는 뚜렷한 방향(예: Stochastic −0.62)을 말하고 있는데도 그렇다 — classifyFull32
    // 는 report.js·2단계와 같은 로직이라 손대지 않는다(브리프 금지). 이 문구가 "왜"를
    // 대신 말한다: 지표가 말이 없는 게 아니라, 전체 판정에 편들 방향 자체가 없다는 뜻.
    ob3FlatNeutralNote: "전체 판정이 중립이라 어느 방향도 편들 수 없습니다 — 지표 자체가 방향을 안 말한 것은 아닙니다. 아래 기여도 숫자를 참고하십시오.",

    // 4단계 — 동의(Task 6). 1·2·3막이 과거였다는 obPastDone 을 받아 "이제부터는 미래"로
    // 여는 전환. 법률 절차가 아니라 이야기의 매듭이 되어야 한다 — 그래서 첫 줄이 "약관"이
    // 아니라 "지금부터 다른 것을 말한다"는 선언이다.
    obFutureOver: "이제부터는 미래를 말합니다",
    obH4: "실제 예측을 보여드리겠습니다",
    obSub4: "다만, 약속이 아닙니다. 시작하기 전에 무엇을 하지 않는지부터 분명히 하겠습니다.",
    obConsentIntro: "MoneyScoop 은 가격·거래량·시간만으로 계산합니다.",
    obConsentNotAdvice: "매수·매도를 권유하지 않습니다.",
    obConsentNoProfit: "수익을 약속하지 않습니다.",
    obConsentLossOwn: "투자 판단과 그 결과에 대한 책임은 본인에게 있습니다.",
    obConsentCheckTxt: "위 내용을 확인했으며, 이 앱의 판정은 참고용 정보이고 투자의 최종 책임은 본인에게 있음에 동의합니다.",

    // 5단계 — 종목 선택 · 분석 시작(Task 6). 고르는 것(state.pick)과 시작하는 것
    // (state.sym, [분석 시작] 클릭)을 문구로도 분리한다 — 헤드라인이 "고르세요"가 아니라
    // "당신 차례"인 이유는, 고르는 행위 자체는 아직 아무것도 실행하지 않기 때문이다.
    obPickH: "이제 당신의 종목입니다",
    obPickSub: "지금까지 본 도구를 이 종목에 적용합니다. 하나를 고르고 아래 버튼을 눌러주세요.",
    obPickSelectedA: "선택: ",
    obPickStart: "분석 시작",
    obPickChecking: "종목을 확인하는 중…",
    // 앞에 종목명을 이어붙여 쓴다(state.symName + ...) — 리뷰 C1: 어느 종목이 확정됐는지
    // 문구에 담아야 화면상 선택(칩)과 실제 확정이 어긋나는 것을 사용자가 눈치챌 수 있다.
    obPickReadySuffix: " 선택을 마쳤습니다. 다음 화면에서 실제 분석이 시작됩니다.",
    // 막다른 골목 금지(설계서 §4.5) — 둘 다 "다른 종목 선택" 버튼과 짝을 이룬다(화면 쪽
    // .ob-retry). 텍스트만 던지고 끝나면 사용자가 갇힌다.
    obPickNotFound: "이 종목을 찾을 수 없습니다.",
    obPickThin: "이 종목은 데이터가 충분하지 않아 지금은 분석할 수 없습니다.",
    obPickRetry: "다른 종목 선택",
    // 리뷰 E(2026-08-19) — commit(data, true)(일반 네트워크 오류·API 층 부재)일 때 붙는
    // 표시. notfound·thin(리뷰 C2)은 이미 실패로 막혔으니, 이 문구가 뜨는 유일한 경우는
    // "종목은 못 확인했지만 진행은 막지 않는다"는 낙관적 폴백뿐이다 — 그 사실을 화면에
    // 남긴다(변명이 아니라 사실: 무엇이 실패했고 지금 보는 게 무엇인지).
    obFallbackNotice: "실제 데이터에 연결하지 못했습니다. 지금부터 보이는 값은 실제 종목이 아니라 예시 데이터입니다.",

    // 6단계 — 실제 분석(설계서 §4.6, Task 7). 옛 "전문분석 체험"(슬라이더 하나, obTut3*)은
    // 이 태스크가 통째로 갈아치웠다 — 그 문구도 함께 지웠다(재사용하지 않는다).
    // 리뷰 I1 — obSub6 이 "5단계에서 고른 종목을…" 으로 **내부 단계 번호**를 불렀다(사용자가
    // 보는 건 숫자 없는 진행 막대뿐이다). 그 자리는 지웠다 — 화면 쪽(screens/onboarding.js
    // step6)이 이 문장 앞에 실제 종목명(state.symName)을 오버라인으로 붙인다, 내부 번호 대신.
    obH6: "이 종목을 실제로 분석합니다",
    obSub6: "32개 도구로 실제로 읽습니다. 오늘 종가부터, 세 지평까지.",
    obTodayLabel: "오늘 종가",
    // 재생(narration) 진행 중 — 값(오늘 종가·세 지평)은 이미 나와 있고, 그 근거만 지금
    // 정리하는 중이다(readingStepper 가 32도구를 하나씩 읽는다).
    obAnalyzing: "근거를 32개 도구로 정리하는 중입니다…",
    // 막다른 골목 금지(단언 7) — runTier("full")이 실패했을 때. 뒤로가기가 없는 단계라
    // "다시 시도" 가 유일하고 정직한 다음 행동이다.
    obAnalysisFailed: "지금은 이 종목을 분석할 수 없습니다.",
    obHzUpMeaning: "상승 방향으로 봅니다.",
    obHzDownMeaning: "하락 방향으로 봅니다.",
    obHzFlatMeaning: "뚜렷한 방향이 없습니다.",
    // "실제 예측"이므로 결과 옆에 약속이 아님을 둔다(설계서 §4.6) — walNoCashValue·
    // rsDisclaimer 와 같은 어간("예측은 약속이 아닙니다")을 그대로 쓴다.
    obHzPromiseNote: "예측은 약속이 아닙니다.",

    // 7단계 — 완료·가격표·지급(설계서 §4.7, Task 8). "방금 받은 것"을 먼저 요약하고(recap),
    // 값을 겪은 뒤에야 가격표를, 가장 마지막에 지급을 보여준다 — 옛 "세 값 비교표"(기본·
    // 심화·전문, state.r3 를 읽던 죽은 셋째 열)는 이 태스크가 통째로 갈아치웠다.
    obDoneH: "받으신 것을 정리하면",
    obDoneSub: "32개 도구 · 3개 지평 · 근거까지, 전부 이 종목으로 실제로 계산됐습니다.",
    // recap(방금 받은 것) — 도구·지평·근거 세 항목이 recapRow() 로 같은 뼈대를 쓴다.
    obRecapHead: "방금 받은 것",
    obRecapSep: " · ",
    obRecapToolsLabel: "개 도구로 판독했습니다",
    obRecapHorizonsLabel: "개 지평을 예측했습니다",
    obRecapEvidenceLabel: "개 반대 의견도 숨기지 않았습니다",
    obRecapEvidenceDetail: "동의만 골라 보여드리지 않습니다 — 근거는 32개 전부 공개됩니다.",
    // 리뷰 M2(Task 8 라운드 1/5) — 반대가 0건일 때 위 문구를 그대로 쓰면 "0개 반대 의견도
    // 숨기지 않았습니다"가 된다. 사실 왜곡은 없지만 "숨길 게 없었다"와 "다 보여줬다"가
    // 겹쳐 읽혀 어색하다 — 없었다는 사실 자체를 자랑도 변명도 아닌 문장으로 새로 쓴다.
    // "개 반대 의견이 있었습니다"는 0/6 어느 쪽에 num 을 앞에 붙여도 자연스럽다(N>0 용
    // 라벨의 "…도 숨기지 않았습니다"는 0에서 "0개도 숨기지 않았다"로 읽혀 어색했다) —
    // num 은 여전히 실제 dissent 개수(0)를 그대로 보여준다(Q1: 값을 가리지 않는다).
    obRecapEvidenceZeroLabel: "개 반대 의견이 있었습니다",
    obRecapEvidenceZeroDetail: "이번 판독엔 반대할 이유 자체가 없었습니다 — 숨긴 게 아니라 없었던 것입니다.",
    obDoneNow: "이제부터",
    // 온보딩 32도구 체험(=심화분석 티어)이 "앞으로도 무료"로 읽히지 않게 가격표 맨 앞에서
    // 분명히 한다(설계서 §4.7·§6) — 이 문장이 obDoneFree(기본은 계속 무료) 보다 먼저 온다.
    obDoneOnboardFree: "방금 보신 심화분석(32개 도구 전체)은 온보딩에서만 무료로 보여드린 것입니다.",
    obDoneFree: "기본분석(5개 도구)은 앞으로도 무료입니다",
