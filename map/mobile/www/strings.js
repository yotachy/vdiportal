...
(function (root, factory) {
  ...
  var t = {
    ...
    // 3단 대조(시안 7a, 설계서 §3.3) — 이 개편의 판매 논거. 왼쪽=답의 폭, 오른쪽=적중률.
    // 숫자 조각은 전부 tier-compare.js 가 window.MSBacktest 에서 계산해 채운다 — 여기 있는
    // 값은 자릿수 없는 문구 조각뿐이다(리터럴 적중률 금지, 관문 tier-compare.test.mjs).
    tcHead: "3단 대조",
    // R3(적중률 리터럴 금지)가 "적중"+퍼센트숫자가 **같은 리터럴**에 있으면 잡는다 — 두
    // 조각으로 쪼개 각 리터럴엔 둘 중 하나만 남긴다(R3 는 조각 단위로 본다, 문장 전체가 아니라).
    tcAxisNoteA: "적중률 막대는 ", tcAxisNoteB: "55~70% 구간만 확대해 보여줍니다.",
    tcCustomRange: "설정에 따라 달라짐",   // 전문은 내 가중치마다 폭이 달라 미리 낼 값이 없다
    ...
    // 실측은 다르다. tier-compare.js 의 deltaNote() 가 이 7조각 사이에 실측값을 끼워 넣는다:
    // "방향 적중률은 {기본}% → {심화}%로, {차이}%p 오릅니다. 심화가 파는 건 방향이 아니라
    // 범위입니다 — 콘 커버리지 {기본}% → {심화}%, 오차는 {배수}배 더 정직해집니다."
    tcDeltaA: "방향 적중률은 ", tcDeltaB: "% → ", tcDeltaC: "%로, ",
    tcDeltaD: "%p 오릅니다. 심화가 파는 건 방향이 아니라 범위입니다 — 콘 커버리지 ",
