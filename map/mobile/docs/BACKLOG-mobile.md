...
- **폴드 모드 전환이 진행 중인 스캔을 버린다** — `app.js` 의 `onMode` 가 `renderShell()` 을 부르면 새 워치리스트
  클로저가 생기고(`scanning=false`) 옛 스캐너는 분리된 DOM 에 계속 쓴다. **Phase 5 부터 있던 것**이고 폴드를
  스캔 도중에 접었다 펴야 재현된다. 스캔 상태를 화면 밖으로 올려야 해소된다.
