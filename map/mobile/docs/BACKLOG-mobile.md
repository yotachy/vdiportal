...
- **스캔 과금(2스쿱)** — ✅ 2026-08-12 적용. 시안 `2c` 의 `Watchlist signal scan 2` 값. 8a 백로그가
  "값이 없다"고 적었던 것은 오독이었다. 30px 아이콘 한 번이 2스쿱을 쓰므로 **확인 시트를 거친다**
  (`MSTierSheet.confirm` — 단계 선택 시트와 껍데기·busy 가드를 공유한다). 한 종목도 못 읽으면
  전량 환급(`SPEC §5`), 일부라도 읽으면 차감 유지(리포트의 주·월봉 누락과 같은 규칙).
