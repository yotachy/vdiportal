...
우선순위 순. 1은 바로 착수 가능, 2·3은 사용자 선행 작업이 있다.

1. **`REASONING · 32 NODES`**(시안 6a) — 지표별 판독문을 Full 에 붙인다. `AGAINST THIS CALL` 이
   이름·기여도까지 냈으니 남은 건 문장이다. **엔진 `*Steps()` 가 한국어라 27종 영어 판독을 새로
   써야 한다** — 이번 패스에서 5종은 `MSLegend` 가 이미 영어로 낸다. 가장 큰 남은 UI 작업.
2. **실기기 확인** — Phase 5·6·7·8a + 오늘 디자인 변경분이 전부 미검증이다. 이 저장소의 검증은
   전부 헤드리스(playwright)라 손맛·성능·터치 타깃은 못 본다.
3. **8b 서버 원장** — `probe-a7f3c2.php` 결과가 유일한 블로커.
...
- **`REASONING · 32 NODES`(시안 6a)** — Full 에 지표별 판독문. `AGAINST THIS CALL` 이 이름·기여도는
  냈으니 남은 건 문장이다. 엔진 `*Steps()` 는 PC 용이라 한국어를 뱉으므로 **27종 영어 판독을 새로
  써야 한다**(5종은 `MSLegend` 가 이미 낸다). ⚠️ 지표별 `analyzeX` 반환 형태가 제각각이라
  `www/indicators.js` 의 `SHAPES` 표를 출처로 쓸 것 — 인자 모양을 틀리면 예외가 아니라 **bias 0** 이
  조용히 나온다(방향이 없는 게 아니라 못 읽은 것인데 화면엔 "중립"으로 보인다).
