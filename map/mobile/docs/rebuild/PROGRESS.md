...
- **페이즈**: P1a (재방문 여정 — 무료 경로: 워치리스트·기본분석 리포트·3단 대조·해제 블록·단계 선택 시트) — ✅ 완료 (Task 1~8)
- **브랜치**: `mobile-rebuild-p1a`
- **마지막 커밋**: Task 8(P1a 마감 — 관문 3중·시안 대조·APK 재빌드·원장 갱신)
- **다음 한 걸음**: **P1b 계획서 작성**(진행 중계·해제 전환·심화 리포트·판독문 32, 시안 19a·8b·8a·18b·19b·20a — `docs/superpowers/specs/2026-08-18-moneyscoop-p1-design.md` §3.5~3.8이 확정 사실). 착수 전 아래 "막힌 것"의 **APK 실기기 확인**을 사용자가 완료해야 한다
...
- **심화·전문분석이 P1b 블록 때문에 잠겨 있다** — 단계 선택 시트(6b)에서 두 티어가 "곧 지원 예정"으로 비활성. 스쿱 비용(3/5)·비용 미리보기("쓰면 12→9")·"가장 많이 씀" 배지가 전부 안 뜬다. P1b가 8블록 심화 리포트를 완성하면 자연히 풀린다
- **`.ms-sheet` 드래그 손잡이 부재** — 시각적 바는 있으나 드래그로 닫는 동작이 없다(P0부터 이월된 항목)
...
| P1a | 재방문 여정 — 무료 경로 (워치리스트·기본분석·3단 대조·해제 블록·단계 선택 시트) | ✅ 완료 | `6d0f6ee..(이 커밋)` | 8태스크(ES2017 재확정·블록 선언 분리·기본 리포트·3단 대조·시트 이관·워치리스트·하드웨어 백·마감), APK 실기기 확인 대기 |
...
- 2026-08-18 · P1a Task 8(마감) · 관문 3중 재검증(`tests/run.sh` 1611건·브라우저 관문 8/8) + 시안 대조(14a·18a·6a·7a·6b — "고칠 것" 2건, 둘 다 지표 빗 잠긴칸 시각 스타일 경미 차이. 3단 대조 문구·읽음 3종·해제 버튼 색 등 나머지 차이는 전부 기존 태스크 보고서·설계서에 근거가 있는 의도적 변경이거나 이미 기록된 부채) + APK 재빌드 확인(신규 변경 없어 기존 12,569,484바이트 그대로) + 본 원장을 P1a 완료로 갱신 · task-8-report.md 상세 · (해시는 이 커밋 자체)
