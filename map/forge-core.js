(function (root, factory) {
  ...
  function analyzeGann(data, opts) {
    ...
    const anchors = opts.draw ? collectAnchors(price, {}).slice(0, _K).map(m => {
  ...
  function analyzeKeltner(data, opts) {
    ...
    const P = price.length;
    if (P < 2) return { mid:0, upper:0, lower:0, pctB:0.5, squeeze:false, bias:0, midArr:[], upperArr:[], lowerArr:[] };
    const midArr = ema(price, len), tr = _trueRangeSeries(candle, price), atrArr = ema(tr, atrLen);
    const upperArr = new Array(P), lowerArr = new Array(P);
    for (let i = 0; i < P; i++) { upperArr[i] = midArr[i] + mult*atrArr[i]; lowerArr[i] = midArr[i] - mult*atrArr[i]; }
    ...
    // 스퀴즈: 현재 밴드폭이 최근 ~20봉 평균폭의 85% 미만이면 변동성 수축(돌파 대기)
    const W = Math.min(20, P), curW = upper - lower;
    let sumW = 0; for (let i = P - W; i < P; i++) sumW += (upperArr[i] - lowerArr[i]);
    const avgW = W ? sumW / W : curW;
    const squeeze = avgW > 0 && curW < 0.85 * avgW;
    return { mid, upper, lower, pctB, squeeze, bias, midArr, upperArr, lowerArr };
  }
  ...
  // Donchian 채널: N봉 롤링 최고가/최저가. 봉별 배열(upperArr/lowerArr/midArr)을 따라가는 '움직이는 채널'로 작도(정적 수평선 금지 — pivot과 구분).
  function analyzeDonchian(data, opts) {
    opts = opts || {};
    const len = opts.len || 20;
    const price = (data && data.price) || (data.candle||[]).map(c=>c.c), candle = (data && data.candle) || [];
    const P = price.length;
    if (P < 2) return { upper:0, lower:0, mid:0, midSlope:0, pos:0.5, bias:0, midArr:[], upperArr:[], lowerArr:[] };
    const upperArr = new Array(P), lowerArr = new Array(P), midArr = new Array(P);
    for (let i = 0; i < P; i++) {
      const s = Math.max(0, i - len + 1);
      let u = -Infinity, l = Infinity;
      for (let j = s; j <= i; j++) {
        const h = candle[j] ? candle[j].h : price[j], lo = candle[j] ? candle[j].l : price[j];
        if (h > u) u = h; if (lo < l) l = lo;
      }
      upperArr[i] = u; lowerArr[i] = l; midArr[i] = (u + l) / 2;
    }
    const upper = upperArr[P-1], lower = lowerArr[P-1], mid = midArr[P-1], last = price[P-1];
    // 이전 봉 중앙선 기울기(방향 보조)
    const midSlope = mid - midArr[P-2];
    const pos = (last - lower) / Math.max(1e-9, upper - lower);   // 0=하단,1=상단
    let bias = (pos - 0.5) * 2 * 0.8 + Math.sign(midSlope) * 0.2;
    bias = Math.max(-1, Math.min(1, bias));
    return { upper, lower, mid, midSlope, pos, bias, midArr, upperArr, lowerArr };
  }
  ...
  // Williams %R: 오실레이터. H/L 필요(data.candle) → 서브패널 없이 hero 배지로만 표시(Phase B 관례).
  ...
  // Aroon: 오실레이터. H/L 필요(data.candle, 신고가·신저가 이후 경과봉) → 서브패널 없이 hero 배지로만 표시(Phase B 관례).
  ...
  function analyzeSMC(candle, opts) {
    ...
    const EMPTY = { fvgs: [], obs: [], bias: 0, last: null, ok: false };
    if (!candle || P < 24) return EMPTY;
    let hasRange = false; for (let i = Math.max(0, P - 40); i < P; i++) { const cd = candle[i]; if (cd && isFinite(cd.h) && isFinite(cd.l) && cd.h > cd.l) { hasRange = true; break; } }
    if (!hasRange) return EMPTY;   // 종가전용(고=저) → SMC 불가
    const last = candle[P - 1].c;
    // FVG(3봉 불균형): 상승 갭 = a.high < c.low, 하락 갭 = a.low > c.high
    const fvgs = [];
    for (let i = 2; i < P; i++) { const a = candle[i - 2], c3 = candle[i];
      if (a.h < c3.l) fvgs.push({ type: "bull", lo: a.h, hi: c3.l, idx: i - 1, filled: false });
      else if (a.l > c3.h) fvgs.push({ type: "bear", lo: c3.h, hi: a.l, idx: i - 1, filled: false });
    }
    for (const g of fvgs) { for (let j = g.idx + 2; j < P; j++) { const cd = candle[j]; if (cd.l <= g.hi && cd.h >= g.lo) { g.filled = true; break; } } }
    // 오더블록: 강한 변위 직전의 반대색 봉(수요/공급 존)
    const atr = _candATR(candle, 14), obs = [];
    for (let i = 3; i < P; i++) { const disp = candle[i].c - candle[i - 1].c; if (atr > 0 && Math.abs(disp) > atr * 1.6) { const bull = disp > 0;
      for (let j = i - 1; j >= Math.max(0, i - 4); j--) { const cd = candle[j], down = cd.c < cd.o; if (bull ? down : !down) { obs.push({ type: bull ? "bull" : "bear", lo: cd.l, hi: cd.h, idx: j, mitigated: false }); break; } }
    } }
    for (const ob of obs) { for (let j = ob.idx + 3; j < P; j++) { const cd = candle[j]; if (cd.l <= ob.hi && cd.h >= ob.lo) { ob.mitigated = true; break; } } }
    const openFvgs = fvgs.filter(g => !g.filled).slice(-5), openObs = obs.filter(o => !o.mitigated).slice(-4);
    let bull = 0, bear = 0;
    for (const z of openFvgs.concat(openObs)) { const mid = (z.lo + z.hi) / 2; if (z.type === "bull") bull += (mid <= last ? 1 : 0.5); else bear += (mid >= last ? 1 : 0.5); }
    let bias = (bull - bear) * 0.22;
    for (const z of openObs.concat(openFvgs)) { if (last >= z.lo && last <= z.hi) bias += z.type === "bull" ? 0.15 : -0.15; }
    bias = Math.max(-1, Math.min(1, bias));
    return { fvgs: openFvgs, obs: openObs, bias, last, ok: true };
  }
  function smcSeries(candle) { return (candle || []).map(() => 0); }   // 방향은 존 기반 drift로 기여
  function smcSteps(smc) {
    if (!smc.ok) return ["SMC(FVG·오더블록)", "실 OHLC 필요 — 티커로 실 캔들 불러오기", "종가전용(붙여넣기) 데이터는 감지 불가", "봉의 고가·저가로 갭·존 판별", "방향 무관(데이터 대기)"];
    const fb = smc.fvgs.filter(g => g.type === "bull").length, fr = smc.fvgs.filter(g => g.type === "bear").length;
    const ob = smc.obs.filter(o => o.type === "bull").length, or = smc.obs.filter(o => o.type === "bear").length;
    return [
      "SMC(FVG·오더블록) 산출",
      "미충족 FVG 상승 " + fb + " / 하락 " + fr,
      "오더블록 수요 " + ob + " / 공급 " + or,
      (smc.bias > 0.1 ? "수요존 우위(지지 유효)" : smc.bias < -0.1 ? "공급존 우위(저항 유효)" : "수요·공급 균형"),
      "종합 방향 " + (smc.bias > 0.1 ? "상승" : smc.bias < -0.1 ? "하락" : "중립") + " (bias " + smc.bias.toFixed(2) + ")"
    ];
  }
  ...
  function run(graph, data, opts) {
    ...
    const keltnerDrift = _kt ? _kt.bias * _prof.trendScale * 0.06 * DW("keltner") : 0;   // 채널 내 위치 방향(±6%)
  ...
  const _VF_MEAN = [-0.05281, -0.02965, -0.04074, -0.0145, 2.61016, 0.20952, 2.4351, 1.81439, -0.06306, 0.49609, 0.94971];
  const _VF_STD = [0.33814, 0.24181, 0.30962, 0.16317, 2.04561, 0.11268, 2.0767, 1.52511, 0.26307, 0.3088, 0.30596];
  const _VF_HZ = [
    { h: 10, lb: "2주", acc: 70, W: [-1.05127, 0.14815, 0.07102, -0.12149, -0.04714, -0.04366, 0.1409, -0.18456, 0.10216, -0.17878, 0.35594], BB: -0.07867 },
  ...
  // ddrisk-horizons/train-ddrisk 검증: 전 지평 OOS 67~69%(다수결·지속성 둘 다 초과). MEAN/STD 공유(피처 동일), W/BB 지평별.
  const _DD_MEAN = [-0.05262, -0.0294, -0.04057, -0.01465, 2.60808, 0.20958, 2.43343, 1.8125, -0.06285, 0.49553];
  const _DD_STD = [0.33838, 0.24202, 0.30996, 0.16301, 2.04907, 0.11275, 2.08013, 1.52676, 0.26328, 0.30888];
  const _DD_HZ = [
    { h: 20, mo: 1, dd: 5, base: 34, acc: 69, W: [-0.0491, -0.05929, -0.09394, -0.01346, 0.31478, -0.01904, 0.24401, 0.30707, -0.02005, 0.05503], BB: -0.78681 },
    { h: 40, mo: 2, dd: 7, base: 35, acc: 68, W: [-0.04308, -0.06753, -0.10359, -0.02853, 0.35366, -0.00849, 0.2608, 0.28872, -0.03167, 0.06018], BB: -0.72909 },
    { h: 60, mo: 3, dd: 9, base: 36, acc: 67, W: [-0.07228, -0.06915, -0.10294, -0.03661, 0.35781, -0.02552, 0.27485, 0.29234, -0.03987, 0.0673], BB: -0.71664 },
  ];
  function forecastDrawdown(price, candle) {
    const x = _riskFeatures(price, candle); if (!x) return null;
    const curve = _DD_HZ.map(z => ({ h: z.h, mo: z.mo, dd: z.dd, base: z.base, acc: z.acc, prob: Math.round(_logit(x, _DD_MEAN, _DD_STD, z.W, z.BB) * 100) }));
  ...
  // 단일봉 급변(shock) 멀티지평 곡선(v1.9.1) — 향후 H봉 내 하루 |수익| > Kσ×현재변동성 발생 확률(갭·쇼크 경보).
  // 지평이 길수록 문턱을 올려 양성률을 균형 유지하는 대각선 곡선: 2주내 2σ / 1달내 2.5σ / 2달내 3σ. 방향 아님 — '큰 하루가 올까'.
  // curve-lab 검증: 대각선 3점 전부 OOS 다수결·지속성 둘 다 초과 + 양성률 44~48% 균형(2σ·10봉 64%·2.5σ·20봉 65%·3σ·40봉 66%).
  // (단기 고문턱 2.5σ/3σ·10봉은 사건 희귀→신기루로 정직 제외.) MEAN/STD 공유(10피처), W/BB 셀별.
  const _SPK_MEAN = [-0.05281, -0.02965, -0.04074, -0.0145, 2.61016, 0.20952, 2.4351, 1.81439, -0.06306, 0.49609];
  const _SPK_STD = [0.33814, 0.24181, 0.30962, 0.16317, 2.04561, 0.11268, 2.0767, 1.52511, 0.26307, 0.3088];
  const _SPK_HZ = [
    { sigma: 2.0, h: 10, lb: "2주", base: 44, acc: 64, W: [0.20168, -0.19332, -0.11134, -0.02109, 0.04248, -0.01198, 0.31282, -0.41799, -0.21332, -0.30666], BB: -0.27476 },
  ...
  // ── 섹터 상대강도(소속 GICS 섹터 ETF 대비) 예보(v1.11) — SPY판보다 강한 상대 방향 축(동료 비교가 노이즈 적음).
  // rel-domain-lab sector 검증: 자명규칙 최강치 대비 +3.0~3.9pp·전/후반 양수·LOSO +3.1~4.0pp·base 49.5%(공정).
  // 피처·정렬은 SPY판과 동일(_relFeats 25피처, 벤치만 섹터 ETF). train-rel-sector.js 산출(전체표본 25,243).
  const _RELS_MEAN = [0.01111, 0.03335, 0.06878, 0.15011, 0.04322, 0.00783, 0.54255, 0.02696, 0.00016, -0.07349, -0.03995, 0.00369, 0.00267, 0.00745, 0.01548, 0.03558, 0.0048, 0.00046, 0.50428, 0.00248, 0.00002, -0.08164, -0.03714, 0.00427, -0.01852];
  const _RELS_STD = [0.08804, 0.1533, 0.23179, 0.38441, 0.1594, 0.02716, 0.33366, 0.16916, 0.14702, 0.08276, 0.31418, 0.30625, 0.07365, 0.12317, 0.17937, 0.2898, 0.12812, 0.02176, 0.30686, 0.12199, 0.12427, 0.07108, 0.31015, 0.30616, 0.23238];
  const _RELS_HZ = [
    { h: 10, lb: "2주", acc: 59, base: 50, W: [0.09357, 0.04443, -0.03215, 0.04929, 0.07545, -0.10195, 0.24746, 0.01807, 0.12376, -0.02363, 0.00937, -0.02587, -0.06046, -0.01279, -0.02157, -0.00918, -0.07214, 0.13298, -0.62833, 0.21352, -0.18584, -0.00417, 0.04205, 0.03818, 0.02959], BB: -0.0047 },
    { h: 20, lb: "1달", acc: 57, base: 50, W: [0.0529, 0.07349, 0.0045, 0.02146, 0.07523, -0.13049, 0.17739, 0.04569, 0.11479, -0.07582, 0.0421, -0.03866, -0.08494, -0.0205, -0.09454, 0.04154, -0.03225, 0.14772, -0.46695, 0.15564, -0.17506, 0.07538, 0.02193, 0.04062, 0.04557], BB: 0.01506 },
    { h: 40, lb: "2달", acc: 56, base: 50, W: [0.06554, 0.14755, -0.08159, 0.05899, 0.09927, -0.18534, 0.08826, 0.02766, 0.13119, -0.12149, 0.02998, -0.01882, -0.11971, -0.10596, -0.04407, 0.05097, -0.02911, 0.14397, -0.33181, 0.18254, -0.20834, 0.1416, -0.01132, 0.05149, 0.03743], BB: 0.01696 },
  ];
  function forecastRelSector(price, sector, etf) {
    const x = _relFeats(price, sector); if (!x) return null;
    const curve = _RELS_HZ.map(z => { const p = _logit(x, _RELS_MEAN, _RELS_STD, z.W, z.BB);
      return { h: z.h, lb: z.lb, prob: Math.round(p * 100), acc: z.acc, base: z.base }; });
    const rep = curve[1];   // 대표=1달(H=20)
    return { prob: rep.prob, outperform: rep.prob >= 50, acc: rep.acc, base: rep.base, etf: etf || null, curve };
  }
  ...
  // kind: osc=별도 축(서브패널·배지)에서 읽는 지표 / overlay=가격 축 위에 얹는 지표
  //       — 앱 배지(app-chart OSC)·PC 분류가 이 필드에서 파생된다(목록 사본 금지).
  ...
  const indicatorRegistry = [
    { id: "ma", label: "이동평균", tier: 1, kind: "overlay", group: "t", input: "price", analyze: analyzeMA },
    { id: "macd", label: "MACD", tier: 1, kind: "osc", group: "m", input: "price", analyze: analyzeMACD },
    { id: "rsi", label: "RSI", tier: 1, kind: "osc", group: "m", input: "price", analyze: analyzeRSI },
    { id: "bollinger", label: "볼린저밴드", tier: 1, kind: "overlay", group: "v", input: "price", analyze: analyzeBollinger },
    { id: "volume", label: "거래량", tier: 1, kind: "osc", group: "q", input: "pv", analyze: analyzeVolume },
    { id: "trend", label: "추세 회귀채널", tier: 2, kind: "overlay", group: "t", input: "price", analyze: analyzeTrend },
    { id: "adx", label: "ADX / DMI", tier: 2, kind: "overlay", group: "t", input: "data", analyze: analyzeADX },
    { id: "stochastic", label: "스토캐스틱", tier: 2, kind: "osc", group: "m", input: "data", analyze: analyzeStochastic },
    { id: "fib", label: "피보나치", tier: 2, kind: "overlay", group: "s", input: "price", analyze: analyzeFib },
    { id: "ichimoku", label: "일목균형표", tier: 2, kind: "overlay", group: "t", input: "price", analyze: analyzeIchimoku },
    { id: "pivot", label: "피벗 S/R", tier: 2, kind: "overlay", group: "s", input: "data", analyze: analyzePivot },
    { id: "psar", label: "PSAR", tier: 2, kind: "overlay", group: "t", input: "data", analyze: analyzePSAR },
    { id: "gann", label: "Gann 부채꼴", tier: 2, kind: "overlay", group: "t", input: "data", analyze: analyzeGann },
    { id: "vwap", label: "VWAP", tier: 3, kind: "overlay", group: "q", input: "pv", analyze: analyzeVWAP },
    { id: "supertrend", label: "슈퍼트렌드", tier: 3, kind: "overlay", group: "t", input: "data", analyze: analyzeSupertrend },
    { id: "atr", label: "ATR", tier: 3, kind: "osc", group: "v", input: "data", analyze: analyzeATR },
    { id: "volumeprofile", label: "볼륨 프로파일", tier: 3, kind: "overlay", group: "q", input: "pv", analyze: analyzeVolumeProfile },
    { id: "structure", label: "시장구조 BOS", tier: 3, kind: "overlay", group: "s", input: "price", analyze: analyzeStructure },
    { id: "keltner", label: "켈트너 채널", tier: 3, kind: "overlay", group: "v", input: "data", analyze: analyzeKeltner },
    { id: "donchian", label: "돈치안 채널", tier: 3, kind: "overlay", group: "v", input: "data", analyze: analyzeDonchian },
    { id: "cci", label: "CCI", tier: 3, kind: "osc", group: "m", input: "data", analyze: analyzeCCI },
    { id: "williams", label: "Williams %R", tier: 3, kind: "osc", group: "m", input: "data", analyze: analyzeWilliams },
    { id: "aroon", label: "Aroon", tier: 3, kind: "osc", group: "t", input: "data", analyze: analyzeAroon },
    { id: "mfi", label: "MFI", tier: 3, kind: "osc", group: "q", input: "cv", analyze: analyzeMFI },
    { id: "elliott", label: "엘리어트 파동", tier: 4, kind: "overlay", group: "s", input: "price", analyze: analyzeElliott },
    { id: "smc", label: "스마트머니 존", tier: 4, kind: "overlay", group: "s", input: "candle", analyze: analyzeSMC },
    { id: "cycle", label: "사이클 위상", tier: 4, kind: "osc", group: "v", input: "price", analyze: analyzeCycle },
    { id: "phasefold", label: "파동 스캔", tier: 4, kind: "osc", group: "v", input: "scan", analyze: null },
    { id: "roc", label: "ROC", tier: 4, kind: "osc", group: "m", input: "price", analyze: analyzeROC },
    { id: "ao", label: "AO", tier: 4, kind: "osc", group: "m", input: "data", analyze: analyzeAO },
    { id: "cmf", label: "CMF", tier: 4, kind: "osc", group: "q", input: "cv", analyze: analyzeCMF },
    { id: "pattern", label: "차트 패턴", tier: 4, kind: "osc", group: "s", input: "data", analyze: analyzePattern }
  ];
