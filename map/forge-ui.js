  ...
  function renderIndRail() {
    ...
    const rowHTML = (t, ic, lv) => { const d = BLOCK_DEFS.find(b => b.type === t) || {}; const on = _evVisible.has(t); const sel = (t === _focusInd); const w = _tw(t); const wf = Math.max(0, Math.min(1, w / 3)); const nm = RAIL_SHORT[t] || d.label || t;
      return `<div class="ir-row${on ? " on" : ""}${sel ? " sel" : ""}" data-irt="${t}" data-lv="${lv}" style="--ic:${ic || "#8a92b2"}"><div class="ir-bar" data-irbar title="${esc(d.label || t)} — 좌우 클릭·드래그 = 가중치(0~3배, 기본 ×1) · 체크박스 = 표시/2차(더블클릭=단독)"><span class="ir-fill" style="width:${(wf * 100).toFixed(1)}%"></span><span class="ir-tick" title="기본 ×1"></span><span class="ir-chk" data-irchk title="표시/2차 토글"></span><span class="ir-lbl">${PATTERN_NATURE.has(t) ? '<span class="ir-tag" title="구조·패턴 감지 지표 — 스윙/기하 패턴(H&S·BOS·플래그 등)"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.6" stroke-linecap="round" stroke-linejoin="round"><path d="M2 17l6-9 5 6 4-8 5 8"/></svg></span>' : ''}${esc(nm)}</span><span class="ir-wval${Math.abs(w - 1) < 0.05 ? " def" : ""}">${Math.abs(w - 1) < 0.05 ? "×1" : "×" + w.toFixed(1)}</span></div><button class="ir-edit" onclick="event.stopPropagation();_railEdit('${t}')" title="파라미터 편집(다시 누르면 닫기)">✎</button></div>`; };
