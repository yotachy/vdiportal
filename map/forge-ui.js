  ...
  function renderSidebar() {
    ...
    const _docRow = d => {
      ...
      const sub = (isFinite(_px) || _tf) ? `<div class="doc-sub">${isFinite(_px) ? `<span class="doc-px">${_hzFmt(_px)}</span>${isFinite(_chg) ? `<span class="doc-chg ${_chg >= 0 ? "up" : "dn"}">${_chg >= 0 ? "▲" : "▼"}${Math.abs(_chg).toFixed(2)}%</span>` : ""}` : ""}${_tf}</div>` : "";
      return `<div class="doc-row${d.id === activeId ? " active" : ""}" data-doc="${d.id}" draggable="true">
         <div class="doc-r1">${_assetHtml(d, "doc-ico")}<span class="doc-nm">${esc(nm)}</span><button class="side-btn" data-docdel="${d.id}" title="목록에서 제거">✕</button></div>${sub}
       </div>`; };
...
<div class="b-hbar b-ebar" style="left:${mx}px;top:${my}px">
  ...
  function boardInit() {
    ...
    <input class="tk-sym" id="tkSym" placeholder="종목 심볼 (예: BTC-USD · AAPL · 005930 국내)" spellcheck="false">
  ...
  <div class="b-zoomctl">
    ...
    if (_tkSym) {
      _tkSym.addEventListener("input", e => { const t = ensureTickerNode(); t.params.symbol = e.target.value.trim(); markDirty(); renderTickerPanel(); });
      _tkSym.addEventListener("keydown", e => { if (e.key === "Enter") { e.preventDefault(); loadTicker(); } });
