// backtest/insider-smallcap-lab.js — 공매도잔고 → 고SI 중소형주 상대방향(vs IJR) 증분 검증
...
const INS = JSON.parse(fs.readFileSync(path.join(__dirname, "shortint-series.json"), "utf8"));
...
for (const sym of kept) {
  ...
  for (let t = START; t <= P.length - Math.max(...HS) - 1; t += STRIDE) {
    ...
    const ins = shortIntFeats(ev, dates[t]);   // filed <= 그 캔들 실제 날짜
...
console.log("=== 공매도잔고 → 고SI 중소형주 상대방향(vs IJR) 증분 검증 ===");
console.log("종목 " + used + " · train " + train.length + " · test " + test.length + " row · 공매도피처 비영 " + insTouched + "행");
console.log("\n(a) 단변량 OOS(신호부호 vs IJR대비 아웃퍼폼 y20):");
["dtcLevel", "chgPct", "siGrowth", "dtcPctile"].forEach((nm, fi) => console.log("   " + nm.padEnd(10) + P(uni(test, fi, 20))));
