...
function runAxis(name,axis){
  ...
  for(const sym of syms){const cds=data[sym].candles,price=cds.map(c=>c.c),hi=cds.map(c=>c.h),lo=cds.map(c=>c.l),op=cds.map(c=>c.o),dates=cds.map(c=>c.t),N=price.length;const ei=eIdx(dates,data[sym].earnings||[]);if(ei.length<8)continue;const tN=tNA(N,ei),sc=sA(N,ei);const loc=[];
    for(let t=WARM;t<=N-H-1;t+=STRIDE){const v=vol10(price,hi,lo,t);if(!v)continue;let bfeat=v;if(axis==="vol"){const g20=gk(op,hi,lo,price,t,20),v20=rvol(price,t,20);bfeat=v.concat([v20?g20/v20:1]);}const ef=earnF(tN,sc,t);let y;if(axis==="spk"){const sv=rvol(price,t,20);if(!sv)continue;y=0;for(let i=t+1;i<=t+H;i++)if(Math.abs(price[i]/price[i-1]-1)>2.5*sv){y=1;break;}}else{const cur=rvol(price,t,H),fwd=rvol(price,t+H,H);if(!cur||!isFinite(fwd))continue;y=fwd>cur?1:0;}
