// backtest/ddrisk-horizons.js — 낙폭리스크 멀티 지평 검증(보유기간별 위험곡선).
  ...
  for (const f of files) {
    ...
    for (let t = WARM; t <= N - MAXH - 1; t += STRIDE) {
      ...
      for (const H of HS) {
        ...
        local[H].push({ x, dd5: lo1 <= -0.05 ? 1 : 0, ddS: lo1 <= -thr ? 1 : 0, _p5: ploLo <= -0.05 ? 1 : 0, _pS: ploLo <= -thr ? 1 : 0 });
      }
    }
    for (const H of HS) { const cut = Math.floor(local[H].length * TRAIN_FRAC); local[H].forEach((r, i) => { r._tr = i < cut; byH[H].push(r); }); }
  }
  const P = x => (x * 100).toFixed(1) + "%";
  console.log("=== 낙폭리스크 멀티지평 검증 (일봉 " + files.length + "종 · OOS · 10피처) ===");
  console.log("진짜=다수결&지속성 둘 다 초과\n");
  for (const H of HS) {
    console.log("── 지평 " + H + "봉 (약 " + Math.round(H / 20) + "개월) ──");
    for (const [lbl, key, base] of [["고정 5% 낙폭", "dd5", "_p5"], ["스케일 " + (5 * Math.sqrt(H / 20)).toFixed(1) + "% 낙폭", "ddS", "_pS"]]) {
