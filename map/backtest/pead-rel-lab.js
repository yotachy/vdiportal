// backtest/pead-rel-lab.js — PEAD → 상대방향(vs SPY) 증분 검증. base=rel25(deployed) vs aug=rel25+pead4.
"use strict";
const fs = require("fs"), path = require("path");
const F = require("./feat-lib.js");
const { peadArray } = require("./pead-feats.js");
const EO = JSON.parse(fs.readFileSync(path.join(__dirname, "earn-ohlc.json"), "utf8"));
const SPYFIX = JSON.parse(fs.readFileSync(path.join(__dirname, "fixtures-shortint", "SPY.json"), "utf8"));
const spyMap = {}; SPYFIX.candle.forEach(c => spyMap[c.t] = c.c);
...
function build() {
  ...
  for (const sym in EO) {
    const cds = EO[sym].candles; if (!cds) continue;
    const closes = cds.map(c => c.c), dates = cds.map(c => c.t), earnings = EO[sym].earnings || [];
    const pead = peadArray(closes, dates, earnings, { win: 45 });
    const P = [], SPY = [], PE = [];
    for (let i = 0; i < cds.length; i++) { const iv = spyMap[dates[i]]; if (iv && closes[i] > 0) { P.push(closes[i]); SPY.push(iv); PE.push(pead[i]); } }
    ...
    for (let t = START; t <= P.length - Math.max(...HS) - 1; t += STRIDE) {
      ...
      const x25 = xo.concat(xr, [betaProxy(P, SPY, t)]), pe = PE[t];
...
console.log("=== PEAD → 상대방향(vs SPY) 증분 검증 (대형주 " + used + "종) ===");
...
console.log("\n=== 판정: " + (pass ? "PASS(PEAD 증분 유의)" : "REJECT(PEAD 증분 불충분 — 대형주·모멘텀 흡수)") + " ===");
