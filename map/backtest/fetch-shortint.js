// backtest/fetch-smallcap.js — 중소형주 OHLC(날짜 t 포함) 수집 → fixtures-shortint/{sym}.json
// shortint-universe.json(마이닝 결과) + IWM·SPY 벤치. forge-api 프록시. 실패/결측 스킵.
"use strict";
const fs = require("fs"), path = require("path");
const API = process.env.BT_API || "https://parksvc.mycafe24.com/map/forge-api.php";
const DIR = path.join(__dirname, "fixtures-shortint");
if (!fs.existsSync(DIR)) fs.mkdirSync(DIR);

const uni = JSON.parse(fs.readFileSync(path.join(__dirname, "shortint-universe.json"), "utf8"));
