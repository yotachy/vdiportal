...
// Task 8(7단계 — 완료·가격표·지급)이 마지막 태스크다. 여기서 "1~7 전부가 등록됐다"를
// 단언해 예외(등록을 잊은 단계)가 영구화되는 것을 막는다 — 부분 집합이 아니라 정확히
// 1~7 전부여야 한다(더 적어도, 더 많거나 중복돼도 실패).
test("APPLIES 가 1~7 을 모두 담는다 — 7단계 재설계가 완주됐다", () => {
  assert.deepStrictEqual([...Q.APPLIES].sort((a, b) => a - b), [1, 2, 3, 4, 5, 6, 7],
    "APPLIES 가 1~7 전부가 아니다 — 어느 단계가 등록에서 빠졌다: " + JSON.stringify(Q.APPLIES));
});
