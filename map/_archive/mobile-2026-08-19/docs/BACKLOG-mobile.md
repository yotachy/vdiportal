...
- **Phase 8c — 구글 로그인 + 익명 계정 병합**(2026-08-15): `device_id` 계정을 구글 계정에 묶어
  8b 가 남긴 한계("재설치하면 5개를 다시 받는다")를 막는다. 마지막 태스크(6)로 지갑 화면에
  로그인/로그아웃 행 하나를 얹었다.
  - **SPEC-economy.md §4 "더 높은 쪽을 취한다"는 일부러 안 지켰다.** `w_merge()`(wallet-lib.php)는
    기기가 아직 아무 구글 계정에도 안 묶여 있으면 그 익명 계정을 그대로 구글 계정으로 승격한다
    (잔량·스트릭·원장 보존, "옮기는" 게 아니라 그 계정이 곧 구글 계정이 된다). 하지만 그 구글
    계정이 **이미 다른 기기에서 존재하는 상태로** 새 기기의 익명 잔량을 또 합칠 때는 SPEC 이
    말한 "높은 쪽 채택"이 아니라 **익명 잔량을 무조건 버린다.** 이유: 높은 쪽을 채택하면 새
    기기에서 익명 5개를 받고 로그인하는 것만으로 잔량이 계속 오르는 래칫이 만들어진다 — SPEC
    자신이 "가장 유력한 남용 경로"라고 지목한 그 재설치 구멍이 병합 규칙을 타고 되살아나는
    것과 같다. 버린 수량은 원장에 남겨("5개가 어디 갔냐"에 답할 근거) `discarded` 로 응답에
    실어 보내고, 화면은 `wMergeDiscarded` 로 그 값을 사용자에게 직접 말한다(조용히 사라지면
    문의로 돌아온다).
  - **`device-claimed`(409)** — 이 기기가 이미 다른 구글 계정에 묶여 있으면 병합을 거절한다
    (안 막으면 두 번째 계정으로 로그인하는 것만으로 첫 사람의 잔량·원장을 통째로 가져간다,
    `device_id` 가 UNIQUE 라 새 계정을 하나 더 만드는 것으로도 못 피한다). **재시도해도 답이
    안 바뀌는 종결 상태**라 화면은 폴링을 멈추고(다음 `setTimeout` 재예약 없음) 재설치가
    유일한 복구임을 말한다(`wDeviceClaimed` — 브리프에 없던 서버 상태라 새로 만든 문구).
  - **`merged`** — 병합으로 넘어간 익명 지갑은 더 못 번다. `w_state()`(wallet-lib.php)가
    `canCheckin:false` 를 서버에서 이미 계산해 주므로, 화면은 그 값을 그대로 따라 출석 행을
    비활성으로 두고 **클릭 리스너 자체를 안 붙인다** — 눌러도 항상 `reason:"merged"` 로
    거절되는 죽은 버튼을 만들지 않는다.
  - **워치리스트는 로그인해도 안 따라온다**(설계서 "동기화 범위" 결정, 8b 이후 스코프 미정으로
    남아 있던 것을 명시적으로 확정). 화면이 이걸 숨기지 않고 로그인 행 바로 아래
    `wWatchlistLocal` 로 적는다 — 잔량은 따라오는데 종목 목록은 안 따라오면 사용자 눈엔 반만
    옮겨진 상태로 보인다.
  - `wallet-screens.test.mjs` 에 DOM 실행 테스트 6종 추가 — watchlist DOM 하네스의 `WlNode`·
    `flush()` 를 재사용하고, `screens/wallet.js` 도 watchlist.js 와 같은 non-UMD 캡처 요령
    (`window=global` 로 잠깐 바꿔 require) 으로 불러온다. 로그인/로그아웃 행 토글 ·
    auth-disabled 행 제거 · 버려진 잔량 안내 · device-claimed 종결 처리(`setTimeout` 스파이로
    다음 폴링 예약이 없음을 직접 확인 — 지연시간을 기다리는 방식은 "언젠가 재시도"하는 회귀를
    못 잡는다) · backend-error 응답의 `authUrl`/`nonce` 미접근 안전성 · merged 출석 행 비활성 +
    클릭 리스너 없음. 4개 뮤테이션(행 제거 대신 비활성화 · device-claimed 인데 계속 폴링 ·
    버려진 잔량 문구 삭제 · merged 인데 출석 제공)을 각각 걸어 대응 테스트가 빨간불로 바뀌는
    것을 실행으로 확인한 뒤 되돌렸다.
  - 테스트 1030 → 1036(`moneyscoop-mobile` 451→457, 다른 다섯 스위트는 무변동).
    `./tests/run.sh concurrency`(같은 구글 계정 8-way 동시 첫 병합에서 계정 행 정확히 1개,
    총원장 5, throw 0) · `dispatcher`(새 op 4검사 + `wallet-auth.php` 2검사) 통과.
...
- **8c 가 남긴 것 — 워치리스트 동기화** — 로그인해도 종목 목록은 기기별로 남는다(현재 화면이
  ...
  "클라이언트는 계산 안 한다" 원칙을 그대로 적용할지부터 정할 것.
- **8c 가 남긴 것 — 애플 로그인** — 구글만 붙였다. iOS 배포를 하게 되면 Sign in with Apple 이
  App Store 심사에서 사실상 강제(다른 소셜 로그인이 있으면 애플도 있어야 한다)다. `wallet-lib.php`
  의 `w_merge()`/`auth_nonce` 는 provider 를 google_sub 하나로 가정하고 있어 구조 변경이 필요하다.
- **8c 가 남긴 것 — 계정 삭제** — 로그인·병합만 있고 탈퇴/데이터 삭제 경로가 없다. 개인정보
  처리방침·스토어 정책(구글 플레이는 앱 내 삭제 경로를 요구)이 걸리는 항목이라 8d 이전에 순서를
  당겨야 할 수 있다.
- **8c 가 남긴 것 — `auth_nonce` 청소 주기 없음**(wallet-lib.php 실측 기록) — `w_nonce_make()` 는
  살아 있는 논스를 재사용하지만, 사용된·만료된 행은 지우지 않는다. 인덱스(`ix_nonce_dev`)가 있어
  당장 조회가 깨지진 않지만 표가 무한정 자란다(로그인 버튼을 누른 횟수만큼) — 실측: 인덱스 없이는
  20만 행에서 호출당 25ms(cafe24 공유 호스팅은 더 느림). 배치 삭제(예: `used=1 and created_at < 기준`)
  스윕을 추가할 것.
