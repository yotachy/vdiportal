...
~~두 제품의 현재 차이는 `forge-scorecard.html` 의 웹↔모바일 비교표가 단일 출처~~ — **2026-08-19 제거됨.** 그 표는 봉쇄된 옛 앱(스쿱 상한 20 · 심화 3 · 전문 5)을 설명하고 있어 지금은 틀린 정보였다. **새 앱의 차이는 [`app-spec.html`](app-spec.html) 이 단일 출처**이며(§1 스코프 · §4-B 경제 · S13 단계), 시안 확정 후 갱신한다. 스코어카드는 이제 **엔진 검증 성적 전용** 문서다.
