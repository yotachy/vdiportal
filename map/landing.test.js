...
test("브랜드명이 상수 1곳에서 주입된다", () => {
  ...
  // [data-brand] 폴백에 브랜드명을 그대로 박아두면(no-JS 이용자가 보는 값) 리네임 시 그 3곳이 stale해진다 —
  // 리터럴은 BRAND 상수 선언 단 한 곳에만 있어야 하고, [data-brand] 요소는 빈 채로 JS가 채워야 한다.
  const naruCount = (html.match(/나루/g) || []).length;
  assert.strictEqual(naruCount, 1, `브랜드 리터럴 "나루"가 ${naruCount}곳에 있음 — BRAND 상수 1곳만 허용, [data-brand] 폴백은 비워야 함`);
});
...
test("검증 방법론이 walk-forward와 기각 공개를 밝힌다", () => {
  const html = read();
  // class="sec sec-alt method" 같은 클래스 문자열 앵커는 클래스 순서만 바뀌어도(예: "sec method sec-alt")
  // 페이지 동작엔 영향 없이 테스트만 조용히 깨진다 — id="verified"/id="how"와 같은 패턴으로 고유 id에 앵커링.
  const i = html.indexOf('id="methodology"');
...
test("투자 유의 안내를 고지한다", () => {
  ...
  // forge.html/forge-scorecard.html/forge-pricing.html 전부 가진 3개 법정 문구 — 랜딩만 짧은 시안 카피를
  // 물려받아 빠져 있었다(가장 노출도 높은 페이지인데). 형제 페이지와 동일 수준으로 못박는다.
  assert.ok(html.includes("등록된 서비스가 아닙니다"), "투자자문업·투자일임업 미등록 고지 없음");
  assert.ok(html.includes("권유"), "매수·매도 권유 아님 고지 없음");
  assert.ok(html.includes("원금 손실"), "원금 손실 위험 고지 없음");
});
...
function colorTokenOf(css, selector) {
  return tokenOf(css, selector, "color");
}

// colorTokenOf의 일반화판 — background 등 다른 색 속성도 뽑아야 하는 쌍(.btn-primary의 배경 등)이 있어 분리.
function tokenOf(css, selector, prop) {
  const block = blockOf(css, selector);
  const re = new RegExp(prop + "\\s*:\\s*var\\((--[\\w-]+)\\)");
  const m = block.match(re);
  assert.ok(m, `${selector}에 ${prop}:var(--token) 선언 없음`);
  return m[1];
}
...
// .ft-disc 한 쌍만 실측하던 기존 커버리지를 CTA 버튼·증거 수치 6개 지점으로 확장.
// 각 요소가 실제로 얹히는 표면(surface/bg — 하드코딩이 아니라 각 섹션 CSS가 쓰는 실제 배경)과,
// 실제 폰트 크기/굵기로 판정한 WCAG 문턱(14px bold는 큰 텍스트 기준 미달 → 4.5:1, kpi-v(34px)만 3:1)을 짝짓는다.
test("주요 CTA·증거 수치가 양 테마에서 WCAG 대비 문턱을 만족한다 (요소별 크기 문턱)", () => {
  const html = read();
  const css = styleCss(html);

  const pairs = [
    { label: ".vitem-v (14px/800 — 큰 텍스트 미달)", fg: tokenOf(css, ".vitem-v", "color"), bg: "--bg", min: 4.5 },
    { label: ".lnk (14px/700 — 큰 텍스트 미달)", fg: tokenOf(css, ".lnk", "color"), bg: "--surface", min: 4.5 },
    { label: ".sec-eyebrow vs --surface (12px/800)", fg: tokenOf(css, ".sec-eyebrow", "color"), bg: "--surface", min: 4.5 },
    { label: ".sec-eyebrow vs --bg (12px/800, .sec how 섹션)", fg: tokenOf(css, ".sec-eyebrow", "color"), bg: "--bg", min: 4.5 },
    { label: ".step-n (12px/800)", fg: tokenOf(css, ".step-n", "color"), bg: "--bg", min: 4.5 },
    {
      label: ".btn-primary 텍스트 vs 기본 배경(14px/700 — 큰 텍스트 미달)",
      fg: tokenOf(css, ".btn-primary", "color"),
      bg: tokenOf(css, ".btn-primary", "background"),
      min: 4.5,
    },
    {
      label: ".btn-primary:hover 텍스트 vs 호버 배경(14px/700 — 큰 텍스트 미달)",
      fg: tokenOf(css, ".btn-primary", "color"),
      bg: tokenOf(css, ".btn-primary:hover", "background"),
      min: 4.5,
    },
    // 큰 텍스트(≥18.66px 또는 14px 이상 굵게가 아니라 실제 24px 이상급)만 3:1 — kpi-v는 34px/800으로 확실히 해당.
    { label: ".kpi-v (34px/800 — 큰 텍스트, 3:1)", fg: tokenOf(css, ".kpi-v", "color"), bg: "--surface", min: 3.0 },
  ];
  ...
  for (const [theme, selector] of [["light", ":root"], ["dark", "[data-theme=dark]"]]) {
    ...
    for (const { label, fg, bg, min } of pairs) {
      ...
      assert.ok(fgHex, `${theme} 테마에 ${fg} 색상값 파싱 실패`);
      assert.ok(bgHex, `${theme} 테마에 ${bg} 색상값 파싱 실패`);
      const ratio = contrastRatio(fgHex, bgHex);
      assert.ok(
        ratio >= min,
        `${theme} 테마: ${label} — ${fg}=${fgHex} vs ${bg}=${bgHex} 대비 ${ratio.toFixed(2)}:1 — 문턱 ${min}:1 미달`
      );
    }
  }
});

// @media 선택자 존재만 확인하던 기존 테스트를 강화 — 블록 안에 실제 레이아웃 조정이 들어있는지까지 확인.
// blockOf는 첫 "}"에서 멎어 다중 규칙 블록엔 못 쓰므로, 중괄호 깊이를 세어 전체 @media 블록을 뽑는 헬퍼를 쓴다.
function atRuleBlock(css, atSelectorRegex) {
  const m = atSelectorRegex.exec(css);
  assert.ok(m, `구간 없음: ${atSelectorRegex}`);
  const openIdx = css.indexOf("{", m.index);
  assert.ok(openIdx >= 0, `여는 중괄호 없음: ${atSelectorRegex}`);
  let depth = 0;
  let i = openIdx;
  for (; i < css.length; i++) {
    if (css[i] === "{") depth++;
...
test("860px 브레이크포인트가 실제로 레이아웃을 조정한다", () => {
  const css = styleCss(read()).replace(/\s+/g, "");
  const block = atRuleBlock(css, /@media\(max-width:860px\)/);
  assert.ok(block.includes(".hd-nav{display:none}"), "860px 구간에서 내비 숨김 규칙 없음");
  assert.ok(block.includes(".hero-h1{font-size:34px}"), "860px 구간에서 히어로 타이틀 축소 규칙 없음");
});

test("마퀴 reduced-motion 구간이 실제로 애니메이션을 끈다", () => {
  const css = styleCss(read()).replace(/\s+/g, "");
  const block = atRuleBlock(css, /@media\(prefers-reduced-motion:reduce\)/);
  assert.ok(block.includes(".mq{animation:none}"), "reduced-motion 구간에 .mq{animation:none} 없음");
});

test("프래그먼트 링크(#...)가 실재하는 id를 가리킨다", () => {
  const html = read();
  // 죽은 링크 테스트가 href="([^"#]...)"로 프래그먼트를 통째 제외해서, id 리네임이 조용히 새는 구멍을 이걸로 메운다.
  const targets = [...new Set([...html.matchAll(/href="#([^"]+)"/g)].map((m) => m[1]))].filter(Boolean);
  assert.ok(targets.length > 0, "프래그먼트 링크가 하나도 없음 — 가드가 무력화됐을 수 있음");
  for (const id of targets) {
    assert.ok(new RegExp(`id="${id}"`).test(html), `프래그먼트 대상 id 없음: #${id}`);
  }
});
...
test("좌측 컬러 accent bar를 쓰지 않는다", () => {
  ...
  // inset 키워드는 값의 맨 앞(inset 4px 0 0 ...) 또는 맨 뒤(4px 0 0 var(--x) inset) 둘 다 유효하므로 양쪽 다 검사.
  const leftBarShadowRe =
    /box-shadow:\s*inset\s+\d+(?:\.\d+)?px\s+0(?:px)?\s+0(?:px)?|box-shadow:\s*\d+(?:\.\d+)?px\s+0(?:px)?\s+0(?:px)?\s+[^;{}]*\binset\b/;
  assert.ok(!leftBarShadowRe.test(html), "inset 좌측 라인 발견");
  ...
  // 한 규칙 안에 함께 있는 흔한 조합(마지막 선언에 세미콜론이 없는 경우 포함)을 겨냥한 휴리스틱이다 —
  // 속성이 여러 규칙에 쪼개지거나 width/height가 px가 아닌 단위(%, em 등)로 지정된 변형은 여전히 놓칠 수 있다.
  const css = styleCss(html);
  ...
  while ((m = pseudoRuleRe.exec(css))) {
    ...
    // 캡처 그룹([^}]*)에는 규칙의 닫는 "}"가 포함되지 않으므로, 마지막 선언이 세미콜론 없이 끝나면
    // [;}] 종결자가 body 안에 없다 — 문자열 끝($)도 종결자로 인정해야 그 경우를 놓치지 않는다.
