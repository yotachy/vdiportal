  ...
  function fcDrawMfi(mfi, reveal) {
    ...
    { const meta = document.getElementById("fcMfiMeta"); if (meta) { const z = mfi.last >= 80 ? ["과열 · 되돌림 주의", "dn"] : mfi.last <= 20 ? ["과매도 · 반등 기대", "up"] : mfi.last >= 50 ? ["자금 유입", "up"] : ["자금 이탈", "dn"]; meta.innerHTML = "MFI " + Math.round(mfi.last) + " <span class='fc-verdit " + z[1] + "'>" + z[0] + "</span>" + '<span class="os-gauge">' + _gaugeSVG(mfi.last, 0, 100, { zones: [{ from: 0, to: 20, color: "#46c28e" }, { from: 80, to: 100, color: "#e06a6a" }], w: 54, h: 30, r: 22 }) + '</span>'; } } _osReveal(c, cw, ch, reveal);
  }
  ...
  function fcDrawWilliams(w, reveal) {
    ...
    { const meta = document.getElementById("fcWilliamsMeta"); if (meta) { const z = w.last >= -20 ? ["과매수 · 되돌림 주의", "dn"] : w.last <= -80 ? ["과매도 · 반등 기대", "up"] : ["중립대", "fl"]; meta.innerHTML = "%R " + Math.round(w.last) + " <span class='fc-verdit " + z[1] + "'>" + z[0] + "</span>" + '<span class="os-gauge">' + _gaugeSVG(w.last, -100, 0, { zones: [{ from: -100, to: -80, color: "#46c28e" }, { from: -20, to: 0, color: "#e06a6a" }], w: 54, h: 30, r: 22 }) + '</span>'; } } _osReveal(c, cw, ch, reveal);
  }
