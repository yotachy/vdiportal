...
- **rows** — 안건. `{id, area, item, pri, date, asis, tobe, result, action, owner, status, done, att, parentId?, updatedAt?}`.
  ...
  - `parentId` — **후속안건(하위레벨 안건)**이면 부모 행 `id`를 가리킴(1단계 깊이). 부모 바로 아래 연한 톤으로 표시. **번호는 영역 내 순번(부모 `1·2·3`, 후속 `1-1·1-2`) — 짧게·`white-space:nowrap`로 줄바꿈 금지**(영역 접두번호 제거, 영역은 헤더/사이드바로 구분). 부모 삭제 시 자식 동반 삭제. **후속안건은 안건·AS-IS·TO-BE 칸을 비활성(흐리게, `td.sub-dim`)** 처리하고 **결정·검토 결과 칸부터 입력**(부모의 현행/대안을 다시 안 적음). ＋추가 시 포커스도 결과 칸으로.
...
- 상태 5종 `STATUSES` = 확정·논의중·확인필요·조치진행·검토완료. **상태·중요도는 PILL 없이 색 글자만**(가독성 우선, 2026-06-17). 참석자(`att`)는 **인원 숫자만**(빈 값은 관리자에게 `＋`), PILL 없음. (범례 칩만 색 배경 유지)
...
- **표는 컨테이너 100% 고정 — 가로 스크롤 없음**(2026-06-17). `table{width:100%; table-layout:fixed}`, `.tbl-scroll{overflow-x:hidden}`, 셀 `word-break:break-word; overflow-wrap:anywhere`(항상 줄바꿈).
  ...
  - **수동 조절(대상만 늘리고 오른쪽 여유가 흡수)**: 머리글 우측 경계 드래그 → **드래그한 컬럼만 커서를 따라 늘고/줄고, 그 차이는 오른쪽 컬럼들이 흡수**(맨 오른쪽부터 여유(현재폭−최소폭)를 소비해 채움). 따라서 **드래그 컬럼의 왼쪽 경계·바로 옆 인접 컬럼은 가능한 한 그대로**(여유가 더 오른쪽에 있으면 인접 컬럼은 폭 유지·위치만 이동), 표 전체 폭 유지·가로 스크롤 없음, 마지막 컬럼엔 핸들 없음. 줄일 땐 남는 폭을 맨 오른쪽 컬럼이 받음. 드래그 중엔 `<col>` px만 직접 갱신, **mouseup 시 현재 렌더된 모든 데이터 컬럼 폭을 그대로 `col.width`+`col.userW=true`로 고정 저장**(합계=컨테이너이므로 재렌더·창 resize에도 안정). `effW(c)=userW?width:autoColPx`. 읽기용 HTML(`layoutRO`)도 동일 방식.
...
- **디자인 방향(엑셀풍·2026-06-17)**: **세로+가로 격자선**(td/th `border-right`+`border-bottom`, `--line`), **중립 회색 헤더**(`--line-faint` 배경·골드 언더라인 제거), 영역 카드 **평탄화**(그림자 없음·`--r-sm`·골드 액센트 제거), 컴팩트(td `8px/1.5`). 골드는 활성/주요 최소만. 안건명만 강조(bold·진한색). 상태·중요도·참석자 PILL 없음. **장식 최소·정보 밀도/통일감 우선**.
