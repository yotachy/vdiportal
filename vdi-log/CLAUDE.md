...
### 허브(`vdi_hub_v1`)
- **cats** — 카테고리(대분류, 2단계, **링크 산출물 전용**). `[{id, name}]`. 기본 `화면정의서` 1개. 순서·이름·삭제는 **카테고리 관리 모달**(`openCatsModal`). 삭제 시 소속 링크 산출물은 첫 카테고리로 이동.
- **items** — 산출물. `{id, catId?, name, type:"builtin"|"link", url?, code?, status?, desc?}`.
  - `type:"builtin"` — 내장 화면. 현재 `관리대장`(id `log`) 하나. **카테고리 밖**(`catId` 없음)에서 사이드바 **상단 고정**(`.nav-pinned`)으로 따로 관리. 삭제·카테고리 지정·링크·코드 불가(제목·상태·설명만 편집). `loadHub`가 과거 구조(관리대장이 카테고리 안)를 자동 마이그레이션(빈 기본 `manage` 카테고리 제거).
  - `type:"link"` — `url`(예: `../deliverables/portal-spec.html`)을 `iframe`으로 미리보기. `code`(VDI-SD-00x)·`status`·`desc` 선택.
  - 상태 3종 `DELIV_STATUSES` = 작성중·검토중·확정(`.dstat-*` 뱃지). 시드: `화면정의서` 카테고리에 통합본 + 개별 14종(VDI-SD-000~014, 설치 확인 포함). `loadHub`가 1회 마이그레이션으로 설치 확인(sd014)을 `guide`→`spec`으로 옮기고 빈 `설치·가이드`를 제거(`_instFix`).
...
- **표시 정렬·계층**: `orderedAreaRows(areaId, ai)`. 정렬 모드 2종(`sortMode()` ← `meta.sortMode`):
  ...
  - 두 모드 모두 각 부모 뒤에 후속안건을 붙이고, `tr.dataset.idx`는 원본 인덱스 유지(편집/삭제 매핑 보존). **화면 render · Excel · 읽기용 HTML 공통**.
