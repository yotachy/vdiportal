...
| 기능 | 비고 |
|---|---|
| 사이드바(영역 내비) | 대시보드·전체 안건·영역 목록(안건수 배지)·화면정의서(관리자). 영역 클릭→단일 영역, 활성 하이라이트(`renderSidebar`/`navTo`). **데스크톱 접기/펼치기**(단일 핸들 `#sideToggle` — 패널 경계 세로 중앙 고정, 아이콘만(‹↔› 회전). 접으면 `body.side-collapsed`로 폭 0 + 핸들이 좌측 가장자리로 슬라이드. `localStorage["vdi_log_sidebar"]` 영속). 모바일(<880px) 햄버거(`#btnNav`·스크림) |
