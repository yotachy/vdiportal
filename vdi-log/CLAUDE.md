...
### 허브(`vdi_hub_v1`)
- **cats** — 카테고리(대분류, 2단계). `[{id, name}]`. 기본은 **사업 단계**: 착수·계획 / 분석 / 화면정의서 / 설계 / 구현 / 시험 / 이행·전개 / 종료·운영(+사용자 추가분은 `기타`). 순서·이름·삭제는 **카테고리 관리 모달**(`openCatsModal`). 삭제 시 소속 링크 산출물은 첫 카테고리로 이동.
- **items** — 산출물. `{id, catId?, name, type:"builtin"|"link", url?, code?, status?, desc?, tier?, owner?, form?}`.
  ...
  - `type:"builtin"` — 내장 화면. `관리대장`(id `log`) 하나. **카테고리 밖**(`catId` 없음), 사이드바 **상단 고정**(`.nav-pinned`), 삭제·링크 불가.
