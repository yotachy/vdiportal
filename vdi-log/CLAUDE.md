...
- **표는 컨테이너 100% 고정 — 가로 스크롤 없음**(2026-06-17). `table{width:100%; table-layout:fixed}`, `.tbl-scroll{overflow-x:hidden}`, 셀 `word-break:break-word; overflow-wrap:anywhere`(항상 줄바꿈).
  ...
  - **수동 조절(인접 컬럼만)**: 머리글 우측 경계 드래그 → **그 컬럼과 바로 오른쪽 인접 컬럼만 폭을 주고받음**(나머지 컬럼은 불변, 두 컬럼 합·표 전체 폭 유지·가로 스크롤 없음, 마지막 컬럼엔 핸들 없음). 드래그 중엔 두 `<col>`의 px만 직접 갱신, **mouseup 시 현재 렌더된 모든 데이터 컬럼 폭을 그대로 `col.width`+`col.userW=true`로 고정 저장**(합계=컨테이너이므로 재렌더·창 resize에도 인접 외 컬럼이 흔들리지 않음). `effW(c)=userW?width:autoColPx`. 읽기용 HTML(`layoutRO`)도 동일 인접 방식.
